...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        $queryUser = "SELECT id, approval_status FROM users WHERE username = :username LIMIT 1";
        ...
        $queryQuotes = "SELECT * FROM quotes WHERE user_id = :user_id ORDER BY id DESC LIMIT :limit OFFSET :offset";
        $stmtQuotes = $db->prepare($queryQuotes);
        $stmtQuotes->bindParam(':user_id', $userId, PDO::PARAM_INT);
